...
class HomeActivity : BaseActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        Flowable.just(
                HttpStoreEvent.getHomeDataEvent(0),
                HttpStoreEvent.getUserEvent(0,userId)
        ).subscribe(this)
    }

    override fun respond(event: Params, flowable: Flowable<*>) {
        when (event.tag) {
            HttpStoreEvent.getHomeData -> flowable.dispatchData<InfoEntity<HomeDataEntity>>().subscribe(s)
            HttpStoreEvent.getUser -> flowable.dispatchData<InfoEntity<UserEntity>>().subscribe(u)
        }
    }
